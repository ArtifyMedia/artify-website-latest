        ...
        <article className="py-12">
          <div className="container mx-auto px-4">
            <div className="max-w-4xl mx-auto">
              <Link
                to="/blog"
                className="inline-flex items-center gap-2 text-gray-400 hover:text-[#C8F31D] transition-colors mb-8"
              >
              ...
              <div className="flex flex-wrap items-center gap-6 text-sm text-gray-500 mb-8 pb-8 border-b border-[#C8F31D]/20">
                <div className="flex items-center gap-2">
                  <div className="w-10 h-10 bg-[#C8F31D]/20 rounded-full flex items-center justify-center">
                    <span className="text-[#C8F31D] font-bold">
                ...
                <button className="ml-auto flex items-center gap-2 text-[#C8F31D] hover:text-[#C8F31D]/80 transition-colors">
                  ...
                  dangerouslySetInnerHTML={{
                    __html: post.content
                      ...
                      .map(line => {
                        ...
                        if (line.startsWith('## ')) return `<h2 class="text-3xl font-bold text-white mt-10 mb-4">${line.substring(3)}</h2>`;
                        if (line.startsWith('### ')) return `<h3 class="text-2xl font-bold text-white mt-8 mb-3">${line.substring(4)}</h3>`;
              ...
              {post.tags && post.tags.length > 0 && (
                <div className="mt-12 pt-8 border-t border-[#C8F31D]/20">
                  <div className="flex items-center gap-3 flex-wrap">
                    <Tag className="w-5 h-5 text-gray-500" />
                    {post.tags.map((tag, idx) => (
                      <span
                        key={idx}
                        className="px-3 py-1 bg-white/5 text-gray-400 text-sm rounded-full border border-[#C8F31D]/20"
                      >
                        {tag}
                      </span>
                    ))}
                ...
                <div className="grid md:grid-cols-3 gap-8">
                  {relatedPosts.map((relatedPost) => (
                    ...
                    >
                      ...
                      <div className="p-6">
                        <h3 className="text-lg font-bold text-white mb-2 group-hover:text-[#C8F31D] transition-colors line-clamp-2">
                          {relatedPost.title}
                        </h3>
